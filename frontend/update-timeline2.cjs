...
// The block to replace starts around line 533 with <div className="screening-progress"
// and ends around line 550.
const matchStart = content.indexOf('<div className="screening-progress"');
if (matchStart !== -1) {
  const matchEndStr = '</div>\\n          ))}';
  const matchEndIndex = content.indexOf('</div>', content.indexOf('))}</div>', matchStart));
  // Wait, let's just find the exact block via substring search if we can't do robust parsing
  const regex = /<div className="screening-progress".*?{STEPS_META\.map[\s\S]*?<\/div>[\s\r\n]*\)\)}[\s\r\n]*<\/div>/m;
  if (regex.test(content)) {
    content = content.replace(regex, replacement);
    fs.writeFileSync(file, content, 'utf8');
    console.log('App.tsx timeline updated using regex.');
  } else {
    console.error('App.tsx regex fail for timeline.');
  }
} else {
  console.error('App.tsx timeline div not found.');
}
