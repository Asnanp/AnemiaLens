  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'0.5rem' }}>
      <div style={{ position:'relative', width:72, height:72 }}>
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--mono)', fontWeight:700, fontSize:'0.8rem', color }}>
          {Math.round(value)}
        </div>
      </div>
      <span style={{ fontSize:'0.58rem', fontFamily:'var(--mono)', color:'var(--text-dim)', letterSpacing:'0.1em', textTransform:'uppercase', textAlign:'center' }}>{label}</span>
    </div>
  );
}

export function QualityView({ quality, onContinue, onBack, loading }: QualityViewProps) {
  const blocking = quality.issues.filter(i => i.severity === 'blocking').length;
  const passed = blocking === 0;

  const ringColor = (v: number) => v >= 75 ? '#10B981' : v >= 40 ? '#F59E0B' : '#EF4444';
  ...
  return (
    <div className="quality-grid" style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:'1.5rem', alignItems:'start' }}>
      ...
      >
        {/* ambient blob */}
        <div style={{ position:'absolute', top:-60, right:-60, width:240, height:240, borderRadius:'50%', background:statusColor, filter:'blur(100px)', opacity:0.08, pointerEvents:'none' }} />
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
              ...
              <div>
                ...
                <p style={{ fontSize:'0.78rem', color:'var(--text-muted)', marginTop:'0.35rem' }}>
                  {passed ? 'Image passed all validation criteria — ready for analysis.' : 'Critical issues detected. Retake recommended for accuracy.'}
                </p>
              </div>
            </div>
            <motion.span
              ...
              style={{
                ...
                fontSize:'0.52rem', fontFamily:'var(--mono)', fontWeight:700, letterSpacing:'0.18em', textTransform:'uppercase',
                background: statusBg, border:`1px solid ${statusBorder}`, color: statusColor,
                boxShadow:`0 0 16px ${statusGlow}`,
                whiteSpace:'nowrap',
              }}
            >{passed ? '✓ READY' : '⚠ ACTION REQUIRED'}</motion.span>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem', marginBottom:'2rem', minHeight:80 }}>
            {quality.issues.length === 0 ? (
              ...
              <motion.div key={i}
                ...
                transition={{ delay:i*0.09, duration:0.4, ease:E }}
                style={{
                  display:'flex', gap:'1rem', alignItems:'flex-start', padding:'1.1rem 1.4rem',
                  borderRadius:'1rem',
              ...
              >
                <div style={{
                  ...
                  fontFamily:'var(--mono)', fontWeight:800, fontSize:'0.9rem',
                }}>{issue.severity === 'blocking' ? '!' : '?'}</div>
                <div>
      ...
      >
        ...
        <div style={{ display:'flex', justifyContent:'space-around', marginBottom:'2rem' }}>
          {metrics.map((m, i) => (
            <motion.div key={m.label} initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 + i*0.1, duration:0.4, ease:E }}>
              <ScoreRing value={m.value} color={ringColor(m.value)} label={m.label} />
            </motion.div>
          ))}
        </div>
          ...
          {metrics.map((m, i) => (
            <div key={m.label}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'0.4rem' }}>
                <span style={{ fontSize:'0.68rem', color:'var(--text-muted)', fontFamily:'var(--mono)' }}>{m.label}</span>
                <span style={{ fontSize:'0.72rem', fontWeight:700, fontFamily:'var(--mono)', color:ringColor(m.value) }}>{Math.round(m.value)}%</span>
              </div>
              <div style={{ height:'3px', background:'rgba(255,255,255,0.06)', borderRadius:'99px', overflow:'hidden' }}>
                <motion.div
                  ...
                  transition={{ duration:1.2, delay:0.35 + i*0.1, ease:E }}
                  style={{ height:'100%', borderRadius:'99px', background:`linear-gradient(90deg, ${ringColor(m.value)}88, ${ringColor(m.value)})`, boxShadow:`0 0 8px ${ringColor(m.value)}66` }}
                />
              </div>
            </div>
          ))}
        ...
        <div style={{ marginTop:'1.5rem', display:'flex', gap:'0.6rem', alignItems:'flex-start', padding:'0.875rem', borderRadius:'0.75rem', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)' }}>
          ...
          <p style={{ fontSize:'0.65rem', color:'var(--text-dim)', lineHeight:1.6, fontStyle:'italic' }}>
            Higher scores improve hemoglobin estimation reliability.
          </p>
        </div>
      </motion.div>
    </div>
