...
export async function getRuntimeStatus(): Promise<RuntimeStatusResponse> {
  const response = await fetch(endpoint('/api/runtime-status'), { method: 'GET' });
...
export async function checkImageQuality(file: File): Promise<QualityAssessment> {
  ...
  const response = await fetch(endpoint('/api/quality-check'), { method: 'POST', body: formData });
  if (!response.ok) {
    let message = 'Image quality check failed.';
    try { const body = await response.json(); if (body?.error) message = body.error; } catch { /* use default */ }
...
export async function analyzeScreening(
  ...
  const response = await fetch(endpoint('/api/analyze'), { method: 'POST', body: formData });
  if (!response.ok) {
