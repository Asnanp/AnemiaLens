...
async def send_email_report(payload: EmailReportRequest):
    """
    Send a screening result summary to the user's email.
    Uses smtplib with Gmail SMTP if configured, otherwise returns demo mode response.
    """
    import smtplib, os
    ...
    hb_line = f"Estimated Hemoglobin: {payload.predicted_hemoglobin:.1f} g/dL" if payload.predicted_hemoglobin else "Hemoglobin: Not available (high uncertainty)"
...
<head><meta charset="utf-8"><style>
  body {{ font-family: Arial, sans-serif; background: #0d0d0d; color: #edede8; margin: 0; padding: 0; }}
  .container {{ max-width: 600px; margin: 0 auto; padding: 32px 24px; }}
  .header {{ border-bottom: 2px solid #C8001E; padding-bottom: 16px; margin-bottom: 24px; }}
  .logo {{ font-size: 22px; font-weight: 700; color: #C8001E; letter-spacing: -0.03em; }}
  .badge {{ display: inline-block; padding: 6px 16px; border-radius: 99px; font-size: 12px;
    font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase;
    background: rgba(200,0,30,0.15); border: 1px solid rgba(200,0,30,0.4); color: #C8001E; }}
  .metric {{ background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08);
    border-radius: 12px; padding: 16px 20px; margin: 12px 0; }}
  .metric-label {{ font-size: 11px; color: #888; text-transform: uppercase; letter-spacing: 0.12em; margin-bottom: 4px; }}
  .metric-value {{ font-size: 20px; font-weight: 700; color: #edede8; }}
  .summary {{ background: rgba(255,255,255,0.02); border-left: 3px solid #C8001E;
    border-radius: 8px; padding: 16px 20px; margin: 20px 0; font-size: 13px; line-height: 1.7; color: #c9c9c0; }}
  .disclaimer {{ font-size: 11px; color: #555; line-height: 1.6; margin-top: 24px;
    padding: 12px 16px; border: 1px solid rgba(255,255,255,0.06); border-radius: 8px; }}
  .footer {{ margin-top: 32px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.06);
    font-size: 11px; color: #444; text-align: center; }}
</style></head>
<body>
<div class="container">
  <div class="header">
    <div class="logo">AnemiaLens</div>
    <div style="font-size:12px;color:#666;margin-top:4px;">Screening Result Report</div>
  </div>
  <div class="badge">{payload.triage_label}</div>
  <div class="metric">
    <div class="metric-label">Anemia Risk Score</div>
    <div class="metric-value">{risk_pct}%</div>
  </div>
  <div class="metric">
    <div class="metric-label">{hb_line.split(':')[0]}</div>
    <div class="metric-value">{hb_line.split(': ', 1)[1] if ': ' in hb_line else '—'}</div>
  </div>
  <div class="summary">{payload.share_text}</div>
  <div class="disclaimer">
    <strong>Important:</strong> This report is generated by an AI screening tool and is NOT a medical diagnosis.
    AnemiaLens is designed as a first-pass screening aid only. Please consult a qualified healthcare professional
    and confirm results with a clinical blood test (CBC) before making any medical decisions.
  </div>
  <div class="footer">
    AnemiaLens · anemia-lens.vercel.app · No lab. No needle. Just a smartphone.
  </div>
</div>
</body>
</html>
"""
    ...
    if not smtp_user or not smtp_pass:
        ...
        return {"status": "demo", "message": f"Demo mode: report would be sent to {payload.email}"}

    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"AnemiaLens Screening Report — {payload.triage_label}"
        msg["From"] = smtp_user
        msg["To"] = payload.email
        msg.attach(MIMEText(payload.share_text, "plain"))
        ...
        return {"status": "sent", "message": f"Report sent to {payload.email}"}
