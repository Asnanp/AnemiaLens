...
GuidanceSource = Literal["qwen", "fallback"]


class GuidanceResult(BaseModel):
    """
    Personalised health guidance generated by an LLM or the rule-based
    ...
    provider_used: str | None = Field(
        default=None,
        description="Inference provider (e.g. 'together', 'groq'), if applicable.",
    )
    ...
    @model_validator(mode="after")
    def _llm_fields_present_when_qwen(self) -> "GuidanceResult":
        if self.source == "qwen" and (
            self.model_used is None or self.provider_used is None
        ):
            raise ValueError(
                "model_used and provider_used must be set when source='qwen'."
            )
        return self
